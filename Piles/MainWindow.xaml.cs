using Piles.Models;
using Piles.ViewModels;
using System.Collections.Generic;
using System.Windows;
using System.Windows.Documents;

namespace Piles
{
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            InitializeComponent();
            Rumination rumination = new Rumination("This is your first task");
            Rumination rumination1 = new Rumination("This is first task of second pile");

            List<Rumination> ruminations = new List<Rumination>();
            List<Rumination> ruminations1 = new List<Rumination>();

            ruminations.Add(rumination);
            ruminations1.Add(rumination1);

            Pile pile = new Pile("General", ruminations);
            Pile pile1 = new Pile("General1", ruminations1);

            List<Pile> piles = new List<Pile>();

            piles.Add(pile);
            piles.Add(pile1);

            Pileup pileup = new Pileup(piles);
            DataContext = new PileupViewModel(pileup);
        }
    }
}
